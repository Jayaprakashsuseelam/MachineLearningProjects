...
This script demonstrates how to train a DQN agent on Atari environments.
It includes CNN architecture, frame stacking, and specialized preprocessing.
"""
...
from dqn.network import CNN_DQNNetwork
from environments.wrappers import make_env, get_env_info
from training.config import ATARI_CONFIG
...
def main():
    """Main function to run the Atari DQN demo."""
    print("=" * 60)
    print("ATARI DQN DEMO")
    print("=" * 60)
    
    # Configuration for Atari
    config = ATARI_CONFIG
    config.env_name = "Breakout-v4"  # Change this to other Atari games
    ...
    try:
        ...
        print("Note: Atari training can take several hours!")
    ...
    print("\nAtari DQN demo completed successfully!")


def create_visualizations(results, test_scores):
    """Create and save visualizations."""
    # Create output directory
    os.makedirs("atari_results", exist_ok=True)
    ...
    plotter.plot_learning_curves(
        ...
        save_path="atari_results/learning_curves.png",
        show=False
    )
    
    # Plot evaluation comparison
    if results['eval_scores']:
        eval_episodes = list(range(0, len(results['episode_rewards']), 100))
        plotter.plot_evaluation_comparison(
            results['eval_scores'],
            eval_episodes,
            save_path="atari_results/evaluation_progress.png",
            show=False
        )
    ...
    plotter.plot_agent_performance(
        test_scores,
        [1000] * len(test_scores),  # Placeholder for episode lengths
        save_path="atari_results/agent_performance.png",
        show=False
    )
    
    print("Visualizations saved to atari_results/")
...
def generate_report(results, test_scores):
    ...
    report = {
        ...
        "test_results": {
            ...
            "improvement_rate": calculate_improvement_rate(test_scores)
        },
        "learning_metrics": {
    ...
    # Save report
    import json
    with open("atari_results/training_report.json", 'w') as f:
        json.dump(report, f, indent=2)
    
    # Print summary
    print("\n" + "=" * 60)
    print("ATARI TRAINING SUMMARY")
    print("=" * 60)
    ...
    print(f"Improvement Rate: {report['test_results']['improvement_rate']:.1%}")


def find_convergence_episode(rewards, window=100):
...
def calculate_improvement_rate(test_scores):
    """Calculate the rate of improvement in test scores."""
    if len(test_scores) < 2:
        return 0.0
    
    # Calculate the percentage of episodes that improved over the previous one
    improvements = sum(1 for i in range(1, len(test_scores)) 
                      if test_scores[i] > test_scores[i-1])
    
    return improvements / (len(test_scores) - 1)


def quick_atari_demo():
    """Quick Atari demo with minimal training."""
    print("Running quick Atari demo...")
    
    # Quick configuration
    config = ATARI_CONFIG
    config.env_name = "Breakout-v4"
    config.max_episodes = 50
    config.eval_frequency = 25
    config.log_frequency = 10
    config.learning_starts = 100  # Start learning earlier for quick demo
    ...
    # Test
    test_scores = trainer.test(episodes=3)
    
    # Print results
    print(f"Quick Atari demo completed!")
...
def test_atari_environment():
    """Test if Atari environment is available."""
    try:
        import gym
        ...
        print("Please install: pip install gym[atari]")
        return False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Atari DQN Demo")
    parser.add_argument("--quick", action="store_true", help="Run quick demo")
    parser.add_argument("--test", action="store_true", help="Test Atari environment")
    args = parser.parse_args()
    
    if args.test:
        test_atari_environment()
    elif args.quick:
        if test_atari_environment():
            quick_atari_demo()
    else:
        if test_atari_environment():
            main()
